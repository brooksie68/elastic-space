...
- JABBERWOCKY (from the engaged draft "Battle Level w the Jabberwocky Rifle", id
  ...
  **2026-09-12 (day, his "i love the stuff you did last night. its killer"): THE WARREN + REINFORCEMENTS + THE FLAYED ONE +
  MUSIC PER LEVEL, built solo on his brief** — every maze ~2.5× (31×28 … 46×40 cells) and it starts in a WARREN (small rooms +
  hallways at the spawn corner; the big rooms, the halls (two on 3–4) and the door always outside it); endless bad guys by
  district (a space's set down → 6 s → a third of it comes back out of sight, forever; PLAY dials Reinforcements / clock;
  corpses capped at 20); the ratling OUT for THE FLAYED ONE (a skinless sprinter, a man's height, meat hook; `creature.mjs`
  is the general pipeline now; 140 cr, balance 1,332 → 1,192); music only on level 1 (`theme.mp3`), level n plays
  `theme-n.mp3` when it exists. Sim 1,156,921 green. Same day, his "better and better": THE ARMOR GLOW (a breathing
  steel-blue glow + a slowly turning shield icon over every pickup) and THE OPTIMIZATION PASS (a torch canvas redrawn every
  frame — 4 ms — fixed; creatures culled past the fog and behind walls; brackets instanced; LRU texture cache; lazy clip
  actions; 575k → 128k triangles and 258 → 173 draw calls at the spawn; `R.profile` / `R.perf` for next time); every level
  starts at full health + armor; THE SOUND LEAK FIXED (his "choppier and choppier… cut out fully on level 3": a media element
  per one-shot, never released — now decoded buffers + freed sources, sound-smoke.mjs); then his "slight motion sickness" →
  his picks of Claude's four: look settle 70 → 25 ms (a LOOK dial), the corner map NORTH-UP (arrow turns, PLAY → Map turns),
  field of view 76 → 88 (the run burst stays, he likes it). AWAITING HIS FLIGHT.
  **JAMES TONIGHT: a track per level — drop `theme-2.mp3` … `theme-6.mp3` into `src/worlds/jabberwocky/assets/audio/`
  (2 catacombs, 3 meat locker, 4 the deep, 5 the middle, 6 the three doors); nothing to wire.**
...
7.5. MOON BATTLE 2100 — **2026-09-12: HIS FIRST FLIGHT VERDICT ("hard time understanding what level I was in, what my objectives were... icons and lettering too tiny") → THE CLARITY ROUND, plan approved whole, BUILT: a SITREP panel (level, target boxes by kind, N of M down, steps, hull/armor/systems, the target), a STRIP MAP along the bottom, a 4 px blinker + readable name on every hostile, the lander's CONDITION (hull 100 + armor from pickups, a SAM = armor stripped + 50 hull, systems degrade at 60/40/25 and repairs fix them), two new shooters (LASER TURRET beams, PELLET GUN bursts; targets 4/6/9), pads with up to three items (HULL +40 / ARMOR +50), SHIFT cycles targets (his pick; Shift no longer burns) + RIGHT-CLICK fires, real weapon drawings + bigger type + a TEXT SIZE dial, a CONTROLS panel on both pages, the tank's MINIMAP + M-map upgrade + direction line, CAMPAIGN → TANK from the start card. Sims 321,057 / 5,312 green; pane-driven. AWAITING HIS FLIGHT. Read both changelogs 2026-09-12 first.** THE WHOLE GAME BUILT 2026-09-11 on James's plan approval ("great plan as usual. rock it, man!"): the name 2075 → 2100; "3 and 3 and out" — the LANDER's three levels (exactly 3 / 4 / 5 targets, SAM sites 1 / 2 / 3 that FIRE — hostile fire round three built at last: warm-up, straight shots with rare corrections, chaff 80%, radar doubles range, bunker door on its own launch, a hit is a crash; the gate pad ends each level; level 3 big — rich deals, a supply on every pad, LIFT OFF from any pad, 0.75× view, THE BASE at the far right: two laser shots for the shield, two missiles for the hull) → CLIMB OUT → the TANK's three levels on a ROUTE of waypoints (a unique landmark beside each, encounters from a deck, a mini boss each — THE WARDEN / THE STRIDER / the siege tank — the base at the end with a hangar that spawns 3 tanks + 3 hovers, two towers, four pits), THE MAP on M, HOVERS + MECH WALKERS (weak frequent beams) + guns on the ground, the hull an ARMOR POOL, pickups (armor / speed / shell speed / armor plate, stack to 3), the grid 80 ft (his 20%), THE END card; the campaign position remembered across both pages. Sims: lander 307,479 green (TEST 16), tank 5,248 green (TESTs 7–9 + 13). AWAITING HIS FLIGHT of everything — read both changelogs 2026-09-11 + both CLAUDE.md START HERE sections first.** The history: (born as LUNAR LANDER, folder `lunar-lander`; RENAMED 2026-09-07 on
