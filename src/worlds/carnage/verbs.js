...
(function () {
  ...
  // ---- moving ----
  add({ id: 'walk', group: G.MOVING, name: 'Walk', facts: [['keys', 'A D'], ['speed', '6 cells a second (the king +30% with variety up)'], ['note', 'a punch slows you to a third while it swings']], stage: (c) => c.place(c.state.city.buildings[0].x0 - 1.5) });
  add({ id: 'climb', group: G.MOVING, name: 'Climb', facts: [['keys', 'W in front of a building grabs it; W S hop one window up or down (each hop finishes on its own)'], ['speed', '3.2 floors a second (the girl +35% with variety up)'], ['note', 'you climb with your back to us, like the cabinet']], stage: (c) => c.place(c.state.city.buildings[0].x0 + 1.5) });
  add({ id: 'sideways', group: G.MOVING, name: 'Sideways on a face', facts: [['keys', 'A D while climbing step one column'], ['edge', 'stepping off the side of a building lets go — a fall'], ['bottom', 'at the ground, S or a sideways step puts you on the street']], stage: (c) => { const b = c.standing(); c.face(b, 1, 2); } });
  ...
  add({ id: 'jump', group: G.MOVING, name: 'Jump', facts: [['keys', 'Space on the street'], ['height', 'about 1.3 floors'], ['catch', 'W in the air catches a building on the way']], stage: (c) => c.place(c.state.city.buildings[0].x0 - 2) });
  add({ id: 'letgo', group: G.MOVING, name: 'The smash (was: let go)', facts: [['keys', 'Space while climbing, on a roof, or in the air'], ['what', 'let go, drop fast, and the landing flattens everything under the fists: soldiers, cars, the truck, the storefront'], ['free', 'three and a half floors of smash cost nothing'], ['what', 'a fall from where you were — free under two floors']], stage: (c) => { const b = c.standing(); c.face(b, 1, 1.5); } });
  add({ id: 'roof', group: G.MOVING, name: 'The roof', facts: [['keys', 'W past the top hoists you onto the roof; S climbs back down; Space smashes down'], ['what', 'walk the roof, jump from it (the blimp is up there some days)'], ['edge', 'walking off drops you the whole height']], stage: (c) => { const b = c.standing(); c.face(b, 1, b.floors - 1.2); } });
  ...
  // ---- round two (2026-09-21): the lanes, the run burst, the stomp ----
  add({ id: 'lane', group: G.MOVING, name: 'Cross the street', facts: [['keys', 'S on the street steps out into the road; W steps back to the buildings'], ['road', 'where the cars and the SWAT truck are: punch them from there; bullets aimed at the other lane miss you'], ['cost', 'a car meeting you in the road crumples (4 health); the truck rams (6 health)']], stage: (c) => c.place(c.standing().x0 + 1.5) });
  add({ id: 'burst', group: G.MOVING, name: 'The run burst', facts: [['keys', 'tap A or D twice inside a quarter second'], ['what', '0.6 s at 2.2× the run, then a skid'], ['bonus', 'soldiers in the way are bowled over: 50 each']], stage: (c) => c.place(c.standing().x0 - 3) });
  add({ id: 'stomp', group: G.PUNCHING, name: 'The stomp', facts: [['keys', 'DOWN arrow on the street'], ['what', 'both fists into the ground: the ground-floor cell under you and its neighbours within reach, every soldier within reach, the bot, the cars if you are in the road']], stage: (c) => c.place(c.standing().x0 + 1.5) });
  // ---- punching ----
  add({ id: 'window', group: G.PUNCHING, name: 'Punch a window', facts: [['keys', 'the ARROWS punch by direction, eight ways (two arrows = a corner); J or click punches straight; hold to keep punching'], ['aim', 'on a face: the window beside, above, below or diagonal; on the street: straight = the floor the fists are at, up = a floor higher, down = the stomp'], ['one punch', 'a window breaks in one; 25 points; what was behind it appears']], stage: (c) => { const f = c.front(); if (f) { f.b.state[f.k] = 0; f.b.deal[f.k] = 'none'; c.refresh(f.b); } } });
  ...
  deal('screamer', 'THE SCREAMER', [['what', 'she appears in the hole, arms up, screaming — a third of the windows on day one, fewer later'], ['hold', 'stay on the cell: 50 points every quarter second up to 1000'], ['eat', 'punch her: 500, +12']]);
  ...
  // ---- the street ----
  add({ id: 'wave', group: G.STREET, name: 'The waves', facts: [['day one', 'nothing until 40 s in AND ten cells broken (dials); then a squad of two from one side, two bursts each, and they pull back'], ['then', 'a lull of ~20 s, the next wave: a squad, the truck (day 2), a sniper in a window (day 2), the drone (day 3), a squad with a cruiser (day 4)'], ['never', 'the same kind twice running']], stage: (c) => { c.C.launchWave(c.state); } });
  add({ id: 'soldier', group: G.STREET, name: 'A soldier', facts: [['comes', 'walks in from an edge, stops 5–8 cells off, kneels, fires bursts with spread, and after his bursts he pulls back'], ['hit', '0.8 health a bullet'], ['punch', 'from the street, within reach ahead: he flies; 50 points'], ['cap', 'two at once on day one, up to five by day nine']], stage: (c) => { c.state.timers.soldier = 0; c.place(c.p.x); } });
