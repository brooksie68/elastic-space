...
(function () {
  ...
  const PROJ = {
    ...
    piranha: (c, t) => { blob(c, 32, 32, 18, 11, '#9aa4aa', 2.5); blob(c, 34, 38, 13, 5, '#c8302a', 0); poly(c, [[14, 32], [2, 22 + Math.sin(t * 25) * 6], [2, 42]], '#9aa4aa', 2.5); blob(c, 42, 28, 3, 3, '#fff', 1.5); blob(c, 43, 28, 1.5, 1.5, INK, 0); c.fillStyle = '#fff'; for (let i = 0; i < 4; i++) poly(c, [[38 + i * 3, 36], [40 + i * 3, 36], [39 + i * 3, 41]], '#fff', 0); },
    ...
    hole: (c) => { blob(c, 32, 34, 30, 20, '#7a6a58', 0); blob(c, 32, 34, 28, 18, '#050308', 3); blob(c, 32, 30, 22, 10, '#000', 0); },
    ...
    vines: (c, t) => { c.strokeStyle = '#1f5a24'; c.lineWidth = 5; for (let i = 0; i < 4; i++) { c.beginPath(); c.moveTo(12 + i * 13, 62); c.quadraticCurveTo(12 + i * 13 + Math.sin(t * 6 + i) * 14, 30, 20 + i * 10, 6); c.stroke(); } c.fillStyle = '#3ac850'; for (let i = 0; i < 5; i++) blob(c, 14 + i * 9, 20 + (i % 2) * 18, 6, 3, '#3ac850', 1.5, 0.6); },
  ...
  const SCAR = {
    ...
    // the splatter kinds (2026-09-10): a fan of spray, long drips, a smear with a handprint dragged through it, a pool with chunks
    bloodspray: (c, R) => { const a0 = R() * TAU; for (let i = 0; i < 40; i++) { const a = a0 + (R() - 0.5) * 1.3, d = 8 + R() * 40, r = 1.2 + R() * 4 * (1 - d / 52); blob(c, 48 + Math.cos(a) * d, 50 + Math.sin(a) * d * 0.8, r * 1.6, r, '#8a1020', 0); } blob(c, 48, 50, 9, 6, '#7a0c1c', 1.5); },
    blooddrips: (c, R) => { splat(c, 48, 34, 18, '#8a1020', R() * 100, 1.5); c.strokeStyle = '#7a0c1c'; c.lineCap = 'round'; for (let i = 0; i < 7; i++) { const x = 30 + R() * 36, len = 16 + R() * 40; c.lineWidth = 2 + R() * 4; c.beginPath(); c.moveTo(x, 36); c.lineTo(x + (R() - 0.5) * 6, 36 + len); c.stroke(); blob(c, x + (R() - 0.5) * 6, 36 + len, 3 + R() * 2, 3 + R() * 2, '#7a0c1c', 0); } },
    bloodsmear: (c, R) => { c.save(); c.translate(48, 50); c.rotate(R() * TAU); c.fillStyle = '#7a0c1c'; c.globalAlpha = 0.75; c.beginPath(); c.ellipse(0, 0, 40, 14, 0, 0, TAU); c.fill(); c.globalAlpha = 1; c.fillStyle = '#8a1020'; c.beginPath(); c.ellipse(6, 2, 10, 12, 0, 0, TAU); c.fill(); for (let i = 0; i < 5; i++) { const fa = -1.2 + i * 0.55; c.beginPath(); c.ellipse(6 + Math.cos(fa) * 16, 2 + Math.sin(fa) * 16 - 6, 3.2, 8, fa + Math.PI / 2, 0, TAU); c.fill(); } c.restore(); },
    bloodchunks: (c, R) => { splat(c, 48, 50, 34, '#8a1020', R() * 100, 2); for (let i = 0; i < 9; i++) blob(c, 30 + R() * 36, 34 + R() * 32, 3 + R() * 4, 2.5 + R() * 3, i % 3 === 0 ? '#e8d8c0' : '#5a0a14', 1, R() * TAU); },
    ...
    hole: (c, R) => { blob(c, 48, 50, 44, 29, '#7a6a58', 0); blob(c, 48, 50, 41, 26, '#050308', 3); blob(c, 48, 46, 30, 16, '#000', 0); c.strokeStyle = '#9a8a74'; c.lineWidth = 2; for (let i = 0; i < 7; i++) { const a = R() * TAU; c.beginPath(); c.moveTo(48 + Math.cos(a) * 42, 50 + Math.sin(a) * 28); c.lineTo(48 + Math.cos(a) * 47, 50 + Math.sin(a) * 31); c.stroke(); } },   // a pale broken rim so it reads from a distance (James 2026-09-10)
    ...
    vines: (c, R) => { c.strokeStyle = '#1f5a24'; c.lineWidth = 5; for (let i = 0; i < 6; i++) { c.beginPath(); c.moveTo(48, 50); c.quadraticCurveTo(48 + (R() - 0.5) * 60, 50 + (R() - 0.5) * 60, 48 + (R() - 0.5) * 90, 50 + (R() - 0.5) * 70); c.stroke(); } },
