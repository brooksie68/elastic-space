...
const KEYORD = ['C','H','N','O','F','Cl','Na','Li','Be','B','Mg','Al','Si','P','S','He','Ne','Ar'];
const KNOWN = {
...
const MADE_LINES = {
  H2O:'That’s water. Two hands on oxygen, two hydrogens, two lone pairs still snug on the ring. The pairs push; that’s why it’s bent.',
  ClNa:'Salt. Nobody shared. One electron moved house, and now they cling by charge.',
  CH4:'Methane. Carbon’s four hands, all shaken. No hands left, so nothing else sticks.',
  H3N:'Ammonia. Three hands used, one lone pair left over. That spare pair is why ammonia is a base.',
  CO2:'Carbon dioxide. Two double bonds, straight as a rod.',
  O2:'Oxygen. A double bond. In real life two of these electrons stay unpaired; the cartoon hides that. We won’t.',
  N2:'Nitrogen. A triple bond, the strongest grip on the shelf. Try pulling it apart.',
  H2:'Hydrogen. The simplest bond there is: one pair, two rings, both full.',
  H2O2:'Hydrogen peroxide. Oxygen holding oxygen, one hand each, hydrogens on the ends.',
  C2H4:'Ethylene. A carbon double bond with four hydrogens. Chain a few thousand and you have a shopping bag.',
};
...
const BREAK = { 1:6, 2:10, 3:13, ionic:11 }; // px at S=1, measured against the drag transient
...
function freeHands(a){ if(a.e.donor && a.outer>0) return 0; return lewis(a).hands - usedHands(a); }
...
function bondLen(sc,b){ return b.ionic ? b.a.R + b.b.R + 6*sc.S : b.a.R + b.b.R - 16*sc.S; }
...
function bondEvent(sc,A,B,kind){
  ...
  if(kind==='covalent'){
    ...
    const w = b.order===1 ? cap(A.e.name)+' and '+B.e.name+': one hand each, paired. Both rings share the pair now.'
          : b.order===2 ? 'Two pairs shared: a double bond. The grip is stronger. Try pulling it apart.'
          : 'Three pairs: a triple bond. Good luck pulling that apart.';
    whisper(sc, 'snap', w);
  } else if(kind==='ionic'){
    sc.effects.push({ type:'fly', from:b.a, to:b.b, t0:sc.t, dur:0.45 });
    sndGliss(b.a.e.pitch, b.b.e.pitch);
    whisper(sc, 'handoff', cap(b.a.e.name)+'’s loose electron left for '+b.b.e.name+'’s empty seat. Both rings are full now. They cling by charge.');
  } else {
    whisper(sc, 'charge', 'Opposite charges. They cling again, no sharing needed.');
  }
  checkMolecule(sc, A);
}
function breakEvent(sc,b){
  ...
  if(b.ionic) whisper(sc, 'apart', 'Pulled apart. The charges stay; bring them back and they cling again.');
  else whisper(sc, 'apart', b.order===1 ? 'Pulled apart. A single bond gives. A triple wouldn’t have.' : b.order===2 ? 'You broke a double bond. That took a real yank.' : 'You broke a triple bond. Show-off.');
}
function bounce(sc,A,B){
  ...
  if(full.bonds.length){ const key = formulaKey(component(full)); const nm = KNOWN[key]; whisper(sc, 'no vacancy', (nm ? cap(nm) : 'That one')+' has no hands left. Nothing sticks.'); }
  else whisper(sc, 'full', cap(full.e.name)+' is full. No hands. Nothing sticks.');
}
...
function drawAtom(sc,a,t){
  ...
  if(a.charge>0){ rc=COL.give; ra=0.55; } else if(a.charge<0){ rc=COL.take; ra=0.55; } else if(complete){ rc=COL.full; ra=0.5+0.14*wob*Math.sin(t*1.6+a.id); }
...
function ledger(a){ const parts = a.e.inner.slice(); parts.push(a.outer+' of '+a.e.seats); let s = parts.join(' · '); // the outer shell reads against its room, so "6 of 8 · wants 2" explains itself
  const fh = freeHands(a); if(a.charge>0) s += ' · +'+a.charge; else if(a.charge<0) s += ' · −'+(-a.charge); else if(isComplete(a)) s += ' · FULL'; else if(canGive(a)) s += ' · GIVES '+a.outer; else if(fh>0) s += ' · WANTS '+fh; return s; }
...
function drawLabels(sc){
  ...
  for(const a of sc.atoms){
    const d=a.R*2;
    ctx.font='600 '+Math.max(11, 0.24*d)+'px "Instrument Sans", sans-serif'; ctx.fillStyle='rgba(255,255,255,0.97)'; ctx.fillText(a.sym, a.x, a.y+0.5);
    ...
    ctx.font='500 '+ds+'px "JetBrains Mono", monospace'; ctx.fillStyle=sc.inkRGBA(0.62);
    ctx.fillText(tracked(ledger(a)), lx, ly+ns*0.62);
    if(sc.xmix>0.01 && sc.xset && sc.xset.has(a)){ ctx.globalAlpha=sc.xmix; ctx.fillStyle=sc.inkRGBA(0.8);
      const cfg = a.e.config || ''; ctx.fillText(tracked(a.e.Z+'p · '+(a.e.Z-a.charge)+'e⁻'+(cfg?' · '+cfg:'')+' · q '+(a.charge>0?'+':'')+a.charge), lx, ly+ns*0.62+ds*1.6); ctx.globalAlpha=1; }
  }
}
...
function draw(sc){
  ...
  drawXray(sc);
  ctx.globalCompositeOperation='source-over'; drawLabels(sc);
}
