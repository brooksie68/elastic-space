...
const CHAPTERS = [
  ...
  { id:'double', title:'Double and triple', tiles:['O','N'], steps:[
    { text:'Two oxygens each want two. Bring them together and they share one pair. Each still has a place open, so drag one oxygen onto the other: a second pair, a double bond.', show:pair('O','O',2), light:['O'], hand:['O','O'], wait:'made:O2', hint:'together, then one onto the other' },
    { text:'Nitrogen has five outer electrons and wants three. Bring two together for one pair, then drag one onto the other twice more: three pairs, a triple bond, one of the strongest there is.', show:pair('N','N',3), light:['N'], hand:['N','N'], wait:'made:N2', hint:'together, then onto the other, twice' },
  ...
  { id:'ions', title:'Ions on their own', tiles:['Na','Cl','H'], steps:[
    { text:'An atom on its own can lose or gain an electron. Sodium holds its one outer electron loosely. Right-click the sodium and choose “lose an electron”: eleven protons, ten electrons, and the balance tips to +1.', show:null, light:['Na'], hand:['Na'], wait:'ion:Na', hint:'right-click the sodium' },
    { text:'A charged atom is an ion. Na⁺ is still sodium, because the protons have not changed, but it is no longer neutral. Chlorine goes the other way: it wants one electron. Right-click the chlorine and choose “gain an electron”: it takes one, seventeen protons against eighteen electrons, −1.', show:null, light:['Na','Cl'], hand:['Cl'], wait:'ion:Cl', hint:'right-click the chlorine' },
    { text:'Opposite charges pull. Bring the two ions together and they hold as salt, and this time no electron has to move: it already did. Right-click either one again, choose “back to neutral”, and its electron comes back.', show:pair('Na','Cl','i'), light:['Na','Cl'], wait:'made:ClNa', hint:'bring the ions together' },
    { text:'Hydrogen is the smallest case. One proton, one electron. Right-click a hydrogen and choose “lose an electron”: nothing is left but the proton. A bare proton is what an acid gives away: acid means loose protons in water.', show:null, light:['H'], hand:['H'], wait:'ion:H', hint:'right-click the hydrogen' },
  ]},
  ...
  { id:'rings', title:'Rings and double bonds', tiles:['C'], steps:[
    { text:'Two carbons can share more than one pair. Give each carbon two hydrogens, so each has two places left. Bring the carbons together for one pair, then drag one carbon onto the other: a second pair, a double bond.', show:{ a:[['C',-0.4,0],['C',0.4,0],['H',-0.8,-0.65],['H',-0.8,0.65],['H',0.8,-0.65],['H',0.8,0.65]], b:[[0,1,2],[0,2,1],[0,3,1],[1,4,1],[1,5,1]] }, light:['C','H'], hand:['C','C','H','H','H','H'], wait:'made:C2H4', hint:'two hydrogens each, carbon to carbon, then one onto the other' },
    ...
    { text:'Now a ring. Give six carbons one hydrogen each, then chain the carbons with single bonds: a single bond can bend. Bring the two ends of the chain together and the ring closes. Every carbon still has one place open, so step every other carbon-to-carbon bond up to a double: three doubles, three singles, around the ring.', show:{ a:[['C',0,-0.8],['C',0.7,-0.4],['C',0.7,0.4],['C',0,0.8],['C',-0.7,0.4],['C',-0.7,-0.4],['H',0,-1],['H',0.95,-0.55],['H',0.95,0.55],['H',0,1],['H',-0.95,0.55],['H',-0.95,-0.55]], b:[[0,1,2],[1,2,1],[2,3,2],[3,4,1],[4,5,2],[5,0,1],[0,6,1],[1,7,1],[2,8,1],[3,9,1],[4,10,1],[5,11,1]] }, light:['C','H'], hand:['C','C','C','C','C','C','H','H','H','H','H','H'], wait:'made:C6H6', hint:'one hydrogen each, chain, close, then every other bond up' },
