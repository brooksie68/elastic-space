...
export class VolumeFire{
 ...
 vec3 v=q.xyz*exp(-dt*.65);float heat=max(0.,q.w-dt*(.85/height+q.w*.4));
 ...
 float tongues=noise(vec3(p.x*11.,p.z*13.,time*1.35));
 float bed=exp(-pow((p.y-.27)/.105,2.)-pow(p.z/.30,4.))*(1.-smoothstep(.70,1.,abs(p.x)));
 float upper=exp(-pow((p.y-(.80-.20*p.x))/.075,2.)-pow((p.z+.05)/.09,2.))*(1.-smoothstep(.67,.88,abs(p.x)));
 float emit=(bed+upper*.65)*smoothstep(.32,.62,tongues)*1.9*drive;
 ...
 vec3 light=vec3(0);float trans=1.;if(enabled>.5&&farT>nearT){float stepSize=(farT-nearT)/48.;float jitter=fract(sin(dot(gl_FragCoord.xy,vec2(12.9898,78.233)))*43758.5453);for(int i=0;i<48;i++){vec3 p=eye+dir*(nearT+(float(i)+jitter)*stepSize);float detail=sin(p.x*35.+time*1.1+sin(p.z*22.))*sin(p.y*28.-time*3.7+sin(p.x*12.));vec3 warp=vec3(sin(p.y*17.-time*2.8+sin(p.z*13.)),0.,cos(p.y*14.-time*2.1+sin(p.x*16.)))*.035*smoothstep(.35,1.2,p.y);float heat=max(0.,temp(p+warp)-.65*(.5+.5*detail));float burn=smoothstep(.60,.83,heat);float opacity=1.-exp(-burn*stepSize*1.7);
 vec3 c=mix(vec3(1.,.055,.001),vec3(1.,.43,.035),smoothstep(.45,1.25,heat));c=mix(c,vec3(1.,.88,.44),smoothstep(1.9,3.2,heat));light+=trans*opacity*c*(.45+heat*.45)*brightness;trans*=1.-opacity*.45;if(trans<.02)break;}}
