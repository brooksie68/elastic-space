...
import {defaults,ranges,sanitize,flicker,zoomFov} from './model.js';
...
const flameFrag=`precision highp float;varying vec2 vUv;uniform float time,seed,brightness;${noiseGLSL}
...
const names={height:'Flame height',speed:'Movement speed',brightness:'Flame brightness',embers:'Ember glow',light:'Firelight',zoom:'Zoom',text:'Text size'};
...
function apply(){cfg=sanitize(cfg);camera.fov=zoomFov(44,cfg.zoom);camera.updateProjectionMatrix();$('panel').style.setProperty('--ui-scale',cfg.text);for(const k in inputs){inputs[k].i.value=cfg[k];inputs[k].o.textContent=cfg[k].toFixed(2);}try{localStorage.setItem('fireplace-study-v1',JSON.stringify(cfg));}catch{}}
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.05);last=now;if(document.hidden)return;if(!paused)clock+=dt*cfg.speed;
 const f=flicker(clock);firelight.intensity=3.8*f*cfg.light;other.intensity=1.8*(.94+.06*Math.sin(clock*4.1))*cfg.light;
 for(const v of flames){v.u.time.value=clock+v.phase;v.u.brightness.value=cfg.brightness;v.m.scale.y=cfg.height*(.89+.11*Math.sin(clock*2+v.phase));}
 ...
 window.fireStudyMetrics={triangles:renderer.info.render.triangles,drawCalls:renderer.info.render.calls,meanFrameMs:frameTimes.reduce((a,b)=>a+b,0)/frameTimes.length,zoom:cfg.zoom,flameLayers:flames.length,paused,logsOnly};}
